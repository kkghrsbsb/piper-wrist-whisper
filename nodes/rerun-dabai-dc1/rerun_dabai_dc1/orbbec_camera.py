...
def main():
    ...
    depth_fx, depth_fy = float(di.fx), float(di.fy)
    depth_cx, depth_cy = float(di.cx), float(di.cy)
    for _event in node:
        try:
            ...
            if ret:
                node.send_output(
                    "image", pa.array(frame), {"primitive": "image", "encoding": "jpeg"}
                )
